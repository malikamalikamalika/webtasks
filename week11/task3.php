    ...
    <div class='top'>
		...
		if (!$mysql) {
			...
			function addNew($mysql,$maker,$model,$price,$year,$url) {
					$sql = "INSERT INTO `cars`(`maker`,`model`,`price`,`year`,`url`) VALUES ('$maker','$model','$price','$year','$url')";
					mysqli_query($mysql,$sql);
					mysqli_close($mysql);
		}
				if (isset($_POST['maker']) && isset($_POST['model']) && isset($_POST['price'])&& isset($_POST['year'])&& isset($_POST['url'])) {
				    $result = addNew($mysql,$_POST['maker'], $_POST['model'], $_POST['price'], $_POST['year'], $_POST['url']);
				}
